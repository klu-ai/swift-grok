...
struct GrokCodeToolRequest: Codable, Equatable {
    ...
    init(from decoder: Decoder) throws {
        ...
        if let decoded = try? container.decode([String: AnyCodable].self, forKey: .arguments) {
            arguments = decoded
        } else if let raw = try? container.decode(String.self, forKey: .arguments),
...
struct GrokCodeFileReadState: Codable, Equatable, Sendable {
    var filePath: String
...
actor GrokCodeReadStateStore {
    private var statesByPath: [String: GrokCodeFileReadState] = [:]

    func record(_ state: GrokCodeFileReadState) {
        statesByPath[state.absolutePath] = state
    }

    func state(for absolutePath: String) -> GrokCodeFileReadState? {
        statesByPath[absolutePath]
    }

    func remove(_ absolutePath: String) {
        statesByPath.removeValue(forKey: absolutePath)
    }
}
...
struct GrokCodeToolUseContext: Sendable {
    ...
    var readState: GrokCodeReadStateStore
    ...
    init(
        ...
        shellTimeoutSeconds: TimeInterval = 120,
        readState: GrokCodeReadStateStore = GrokCodeReadStateStore()
    ) {
...
protocol GrokCodeTool {
    ...
    var schema: GrokCodeToolSchema { get }
...
struct AnyGrokCodeTool {
    ...
    let schema: GrokCodeToolSchema
    let isConcurrencySafeByDefault: Bool
    let requiredPermission: GrokCodeToolPermission

    private let decodeInput: ([String: AnyCodable]) throws -> Any
    private let validateInput: (Any, GrokCodeToolUseContext) throws -> Void
    private let checkToolPermission: (Any, GrokCodeToolUseContext) async throws -> GrokCodePermissionDecision
    private let runTool: (Any, GrokCodeToolUseContext) async throws -> GrokCodeToolResult
...
extension GrokCodeToolResult {
    func capped(to limit: Int) -> GrokCodeToolResult {
        ...
        for block in content {
            ...
            case .json:
                let encoded = (try? JSONEncoder().encode(block)).map(\.count) ?? limit
